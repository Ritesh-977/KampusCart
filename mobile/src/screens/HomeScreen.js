...
import { ThemeContext } from '../context/ThemeContext'; // Ensure this path is correct for your project
...
const SpotlightCard = ({ item, onPress }) => {
  ...
  const spStyles = useMemo(() => StyleSheet.create({
    card: { width: SPOT_W, height: 215, borderRadius: 22, overflow: 'hidden',
      ...
      shadowColor: theme.primaryAction, shadowOffset: { width: 0, height: 8 },
      shadowOpacity: 0.3, shadowRadius: 16, elevation: 12 },
    img: { ...StyleSheet.absoluteFillObject },
    noImg: { ...StyleSheet.absoluteFillObject, alignItems: 'center', justifyContent: 'center', backgroundColor: theme.card },
    topRow: { position: 'absolute', top: 14, left: 14 },
    badge: { backgroundColor: theme.primaryAccent + 'C0', borderRadius: 20, paddingHorizontal: 11, paddingVertical: 5,
      borderWidth: 1, borderColor: theme.primaryAccent + '65' },
    ...
    price: { fontSize: 16, fontWeight: '800', color: theme.secondaryAction },
  ...
  return (
    <Press onPress={onPress} style={spStyles.card}>
      {img
        ...
        : <View style={[spStyles.img, spStyles.noImg]}><Ionicons name="image-outline" size={36} color={theme.textTertiary} /></View>
      }
...
const EventsCalendar = ({ events = [], navigation }) => {
  ...
  const evStyles = useMemo(() => StyleSheet.create({
    ...
    title: { fontSize: 18, fontWeight: '900', color: theme.textMain },
    fullBtn: { fontSize: 13, fontWeight: '800', color: theme.primaryAccent },
    ...
    cardTime: { fontSize: 11, color: theme.textTertiary, fontWeight: '600' },
    cardTitle: { fontSize: 13, fontWeight: '800', color: theme.textBody, marginBottom: 6 },
    cardLocRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
    cardLoc: { fontSize: 11, color: theme.textTertiary, fontWeight: '500', flex: 1 },
  }), [theme]);
        ...
        <>
          {featured && featFmt && (
            <TouchableOpacity activeOpacity={0.85} onPress={() => navigation.navigate('EventDetails', { event: featured })} style={evStyles.featured}>
              <View style={evStyles.featuredOverlay} />
          ...
          {upcoming.length > 0 && (
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 12, paddingRight: 20 }}>
              {upcoming.map(e => {
                ...
                const color = e.color || theme.primaryAccent;
                return (
                  <TouchableOpacity
                    key={e._id}
                  ...
                  >
                    <View style={[evStyles.cardDate, { backgroundColor: color + '22', borderColor: color + '50' }]}>
...
const HomeScreen = ({ navigation }) => {
  ...
  const QUICK = useMemo(() => [
    { label: 'Books',       icon: 'book',           color: theme.primaryAccent, glow: theme.primaryAction, cat: 'Books & Notes' },
    { label: 'Electronics', icon: 'phone-portrait', color: theme.secondaryAction, glow: '#10b981', cat: 'Electronics' },
  ...
  const FEATURES = useMemo(() => [
    { label: 'Campus Events',  icon: 'calendar',       color: theme.primaryAccent, bg: theme.card, desc: 'Fests, seminars & more' },
    { label: 'Study Material', icon: 'document-text',  color: theme.secondaryAction, bg: theme.card, desc: 'Notes, PDFs & resources' },
    { label: 'Exam Schedule',  icon: 'clipboard',      color: '#fb923c', bg: theme.card, desc: 'Mid-sems & end-sems' },
    { label: 'Sports',         icon: 'trophy',         color: '#38bdf8', bg: theme.card, desc: 'Register & compete' },
  ], [theme]);
  ...
  const mainStyles = useMemo(() => StyleSheet.create({
    ...
    campusBadge: { flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: theme.card, borderRadius: 16,
      ...
      shadowColor: theme.primaryAction, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
    ...
    suggestBox: { backgroundColor: theme.card, borderRadius: 16, marginTop: 6, borderWidth: 1, borderColor: theme.inputBorder,
      ...
      shadowOpacity: 0.5, shadowRadius: 16, elevation: 20, zIndex: 999 },
    ...
    soonBadge: { backgroundColor: theme.primaryAccent + '1F', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4,
      borderWidth: 1, borderColor: theme.primaryAccent + '40' },
    soonText: { fontSize: 11, color: theme.primaryAccent, fontWeight: '800' },
    ...
    // REMOVED: featureBlob style entirely to clean up code
    ...
    <View onLayout={(e) => { headerHeightRef.current = e.nativeEvent.layout.height; }}>
        ...
        <View style={mainStyles.quickRow}>
          {QUICK.map((q) => (
            <Press key={q.label} onPress={() => handleQuick(q.cat)} style={mainStyles.quickItem}>
              <View style={[mainStyles.quickCircle, { borderColor: q.glow + '60', shadowColor: q.glow }]}>
      ...
      <Animated.View style={[mainStyles.section, makeAnim(sec3Anim)]}>
        <View style={mainStyles.sectionHead}>
          ...
          <View style={mainStyles.soonBadge}>
            <Text style={mainStyles.soonText}>Coming Soon</Text>
          </View>
        </View>
        <View style={mainStyles.featuresGrid}>
          {FEATURES.map((f) => (
            <Press key={f.label} onPress={() => handleFeature(f.label)} style={[mainStyles.featureCard, { backgroundColor: f.bg }]}>
              {/* REMOVED the featureBlob View here */}
              ...
              {['Study Material', 'Sports', 'Campus Events'].includes(f.label) ? (
                <View style={[mainStyles.featureSoon, { borderColor: f.color + '50' }]}>
              ...
              return (
                <Press key={item._id} onPress={() => handleItemPress(item)} style={mainStyles.recentCard}>
                  {img
                    ...
                    : <View style={[mainStyles.recentThumb, mainStyles.recentThumbFallback]}><Ionicons name="cube-outline" size={18} color={theme.textTertiary} /></View>
                  }
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <Press key={cat.label} onPress={() => setActiveCategory(cat.label)} style={[mainStyles.chip, active && mainStyles.chipOn]}>
                <Ionicons name={cat.icon} size={13} color={active ? '#fff' : theme.textTertiary} style={{ marginRight: 5 }} />
  ...
  return (
    <SafeAreaView style={mainStyles.root} edges={['top']}>
      <StatusBar backgroundColor={theme.background} barStyle={theme.statusBarStyle === 'light-content' ? 'light-content' : 'dark-content'} />
      ...
      <Animated.View style={[mainStyles.topBar, makeAnim(heroAnim, 18)]}>
        ...
        <View style={mainStyles.heroTop}>
          ...
          <Press onPress={() => setModalVisible(true)} style={mainStyles.campusBadge}>
            ...
            <Ionicons name="chevron-down" size={12} color={theme.primaryAccent} />
          </Press>
        </View>

        <View style={mainStyles.searchWrap}>
          <Ionicons name="search" size={17} color={searchFocused ? theme.primaryAccent : theme.textTertiary} style={{ marginRight: 9 }} />
          <TextInput
                  ...
                  {img
                    ...
                    : (
                      <View style={[mainStyles.suggestThumb, mainStyles.suggestThumbFallback]}>
                        <Ionicons name="cube-outline" size={16} color={theme.textTertiary} />
                      </View>
                    )
                  }
                  <View style={{ flex: 1 }}>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[theme.primaryAccent]} tintColor={theme.primaryAccent} />
          }
            ...
            <View>
              {!loading && sortedItems.length > 8 && (
                <Press onPress={() => setShowAll(v => !v)} style={mainStyles.viewAllBtn}>
                  ...
                  <Ionicons name={showAll ? 'chevron-up' : 'chevron-down'} size={15} color={theme.primaryAccent} />
                </Press>
              )}
          ...
          ListEmptyComponent={
            <View style={mainStyles.emptyBox}>
              <View style={mainStyles.emptyOrb}>
                <Ionicons name="storefront-outline" size={44} color={theme.primaryAccent} />
              </View>
        ...
        <View style={mainStyles.modalBg}>
          <View style={mainStyles.sheet}>
            ...
            <View style={mainStyles.mSearch}>
              <Ionicons name="search" size={15} color={theme.textTertiary} style={{ marginRight: 8 }} />
              <TextInput
                ...
                return (
                  ...
                  >
                    ...
                    <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                      ...
                      {isActive && <Ionicons name="checkmark-circle" size={20} color={theme.primaryAccent} />}
                    </View>
                  </TouchableOpacity>
                );
              }}
